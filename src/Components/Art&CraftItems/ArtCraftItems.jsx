...
const ArtCraftItems = () => {
  ...
  useEffect(() => {
    setArtCraftItems(initialData); // Set the initial data when available
  }, [initialData]);

  useEffect(() => {
    sortTouristSpots();
  }, [sortOrder, artCraftItems]); // Added sortOrder and touristSpots as dependencies

  const sortTouristSpots = () => {
    if (!artCraftItems.length) return; // Return if touristSpots is empty
    let sorted = [...artCraftItems];
    if (sortOrder === "asc") {
